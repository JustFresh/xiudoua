			...
			<div class="container-fluid">
				<div class="row">
					<div class="col-sm-12 col-md-12">
						<div class="tab-nav">
							<ul class="pull-left">
								<li class="selected"><a href="<%=basePath %>admin/index.htm?mod=cms&act=channels">栏目管理</a></li>
							</ul>
							<div class="opts pull-right">
								<a href="<%=basePath %>admin/index.htm?mod=cms&act=channels&opt=channel_mng" class="btn btn-success"><i class="fa fa-plus"></i> 添加栏目</a>
							</div>
					...
					<div class="col-sm-12 col-md-12">
						<div class="search-form pull-right">
							<form action="<%=basePath %>admin/index.htm" method="get">
								...
								<input type="hidden" name="act" value="channels" />
								...
								<span class="form-element">
									...
									<select name="parentId" class="input-select">
										...
										<c:forEach var="model" items="${channelList }" varStatus="status">
											<option value="${model.id }" <c:if test="${model.parentId==channel.id }">selected</c:if>>${model.name }</option>
										</c:forEach>
									</select>
								</span>
								<span class="form-element">
						...
						<table class="table table-striped table-bordered">
							...
							<tbody>
								<c:if test="${page != null && page.list != null}">
									<c:forEach var="channel" items="${page.list}" varStatus="status">
										<tr>
											<td width="30px"><input type="checkbox" value="${channel.id }" name="channelIds" /></td>
											<td>${channel.name }</td>
											<td><img src="<%=basePath %>${channel.thumb}" style="max-height: 25px;"/></td>
											<td>
												...
												<c:if test="${channel.cType == '4'}">
													文件下载
												</c:if>
											</td>
											<td>
												<c:if test="${channel.isNav == '1'}">
													是
												</c:if>
												<c:if test="${channel.isNav == '-1'}">
													否
											...
											<td>${channel.level }</td>
											<td>
												<jsp:setProperty name="dateValue" property="time" value="${channel.createTime}"/>
											...
											<td>
												<c:if test="${channel.status != null && channel.status == 1}">
													<a href="javascript: void(0)" onclick="changeStatus(${channel.id},-1)" class="btn-outline btn-outline-danger">禁用</a>
												</c:if>
												<c:if test="${channel.status != null && channel.status == -1}">
													<a href="javascript: void(0)" onclick="changeStatus(${channel.id},1)" class="btn-outline btn-outline-info">启用</a>
												</c:if>
												<a href="<%=basePath %>admin/index.htm?mod=cms&act=channels&opt=channel_mng&id=${channel.id}" class="btn-outline btn-outline-success">编辑</a>
												<a href="javascript: void(0)" onclick="removeFn(${channel.id})" class="btn-outline btn-outline-danger">删除</a>
											</td>
										</tr>
									</c:forEach>
								</c:if>
				...
				<div class="row">
					<div class="col-sm-12 col-md-12">
						<div class="bottom-opts">
							<input type="checkbox" name="checkall" onclick="changeCheckboxStatus('channelIds')" />
							<a href="javascript: void(0)" onclick="batchRemove('channelIds','channel')" class="btn-outline btn-outline-default">批量删除</a>
						</div>
					</div>
				</div>
			</div>
