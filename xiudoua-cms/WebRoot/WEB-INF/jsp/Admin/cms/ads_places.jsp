					...
					<div class="col-sm-12 col-md-12">
						<div class="tab-nav">
							<ul class="pull-left">
								...
								<li class="selected"><a href="javascript: void(0)">广告位管理</a></li>
							</ul>
							<div class="opts pull-right">
								<a href="<%=basePath %>admin/index.htm?mod=cms&act=ads&opt=ads_place_mng" class="btn btn-success"><i class="fa fa-plus"></i> 添加广告位</a>
							</div>
					...
					<div class="col-sm-12 col-md-12">
						<div class="search-form pull-right">
							<form action="<%=basePath %>admin/index.htm" method="get">
								...
								<input type="hidden" name="opt" value="ads_places" />
								...
								<span class="form-element">
									...
									<select name="status" class="input-select">
										...
										<option value="1" <c:if test="${adsPlace.status==1 }">selected</c:if>>--启用--</option>
										<option value="-1" <c:if test="${adsPlace.status==-1 }">selected</c:if>>--禁用--</option>
									</select>
								</span>
								<span class="form-element">
									<label>分页数：</label>
						...
						<table class="table table-striped table-bordered">
							...
							<tbody>
								<c:if test="${page != null && page.list != null}">
									<c:forEach var="adsPlace" items="${page.list}" varStatus="status">
										<tr>
											<td width="30px"><input type="checkbox" value="${adsPlace.id }" name="adsPlaceIds" /></td>
											<td>${adsPlace.name }</td>
											<td>${adsPlace.width }</td>
											<td>${adsPlace.height }</td>
											<td>
												<c:if test="${adsPlace.status == '1'}">
											...
											<td>
												<c:if test="${adsPlace.status != null && adsPlace.status == 1}">
													<a href="javascript: void(0)" onclick="changeStatus(${adsPlace.id},-1)" class="btn-outline btn-outline-danger">禁用</a>
												</c:if>
												<c:if test="${adsPlace.status != null && adsPlace.status == -1}">
													<a href="javascript: void(0)" onclick="changeStatus(${adsPlace.id},1)" class="btn-outline btn-outline-info">启用</a>
												</c:if>
												<a href="<%=basePath %>admin/index.htm?mod=cms&act=ads&opt=ads_place_mng&id=${adsPlace.id}" class="btn-outline btn-outline-success">编辑</a>
												<a href="javascript: void(0)" onclick="removeFn(${adsPlace.id})" class="btn-outline btn-outline-danger">删除</a>
											</td>
										</tr>
									</c:forEach>
								</c:if>
				...
				<div class="row">
					<div class="col-sm-12 col-md-12">
						<div class="bottom-opts">
							<input type="checkbox" name="checkall" onclick="changeCheckboxStatus('adsPlaceIds')" />
							<a href="javascript: void(0)" onclick="batchRemove('adsPlaceIds','adsPlace')" class="btn-outline btn-outline-default">批量删除</a>
						</div>
					</div>
				</div>
			</div>
